...
namespace backend_discTests
{
    ...
    public sealed class IntegrationTestsDb
    {
        ...
        public async Task CleanupCreatedEmployee()
        {
            if (createdEmployee != 0)
            {
                var employee = await _context.Employees.FindAsync(createdEmployee);
                if (employee != null)
                {
        ...
        public async Task AddEmployeeSPAsync_Not_UniqueValues(string cpr, int deptId, int discId, int posId, int userRoleId, string firstName, string lastName, string imagePath, string pwHash, string username, string privateEmail, string privatePhone, string workEmail, string workPhone)
        {
            AddEmployeeSpParams p = new AddEmployeeSpParams
            {
                CPR = cpr,
        ...
        [TestMethod]
        [DataRow(
            null, VALID_DEPARTMENT_ID, VALID_DISC_PROFILE_ID, VALID_FIRST_NAME, VALID_LAST_NAME, VALID_POSITION_ID, DEFAULT_IMAGE_PATH, PASSWORD_HASH, VALID_USERNAME, VALID_PRIVATE_EMAIL, VALID_PRIVATE_PHONE, VALID_WORK_EMAIL, VALID_WORK_PHONE, VALID_USER_ROLE_ID
        )]
        [DataRow(
