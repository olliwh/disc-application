...
GO use disc_profile_relational_db;
CREATE TABLE departments (
    id INT PRIMARY KEY IDENTITY(1, 1),
    name NVARCHAR(50) NOT NULL,
    description NVARCHAR(255)
);
GO CREATE TABLE disc_profiles (
        id INT PRIMARY KEY IDENTITY(1, 1),
        name VARCHAR(255) NOT NULL,
        color CHAR(7) NOT NULL,
        description VARCHAR(255) NOT NULL
    );
GO CREATE TABLE positions (
        id INT PRIMARY KEY IDENTITY(1, 1),
        name VARCHAR(255) NOT NULL,
        description VARCHAR(255) NOT NULL
    );
...
GO CREATE TABLE employee_private_data (
...
GO CREATE TABLE projects_audit(
        id INT PRIMARY KEY IDENTITY(1, 1),
...
GO CREATE TABLE completion_intervals (
        id INT PRIMARY KEY IDENTITY(1, 1),
        time_to_complete NVARCHAR(50) NOT NULL
    );
GO CREATE TABLE project_tasks (
        id INT PRIMARY KEY IDENTITY(1, 1),
        name NVARCHAR(50) NOT NULL,
        completed BIT NOT NULL,
...
GO CREATE TABLE stress_measures (
        id INT PRIMARY KEY IDENTITY(1, 1),
        description NVARCHAR(255),
        measure INT,
...
GO CREATE TABLE project_tasks_employees (
        task_id INT NOT NULL,
        employee_id INT NOT NULL,
...
GO CREATE TABLE employees_projects (
        project_id INT NOT NULL,
        employee_id INT NOT NULL,
        currently_working_on BIT NOT NULL,
        is_project_manager BIT NOT NULL PRIMARY KEY (project_id, employee_id),
        FOREIGN KEY (project_id) REFERENCES projects(id),
        FOREIGN KEY (employee_id) REFERENCES employees(id)
    );
GO -- Table needs own PK because composite would not be unique
...
GO CREATE
    OR ALTER TRIGGER add_to_project_audit_table ON projects
AFTER
INSERT AS BEGIN
SET NOCOUNT ON;
...
GO -- Create trigger that marks task as completed with time of completion when time_to_finish has been set
    CREATE
    OR ALTER TRIGGER trg_task_completion ON project_tasks
AFTER
UPDATE AS BEGIN
SET NOCOUNT ON;
UPDATE pt
...
WHERE d.time_to_complete_id IS NULL
    AND i.time_to_complete_id IS NOT NULL;
END;
GO CREATE
    OR ALTER VIEW employees_own_profile AS
SELECT e.id,
    e.work_email,
    e.work_phone,
...
GO
select *
from employees_own_profile;
-- Stored function
DROP FUNCTION IF EXISTS ProjectEmployeeStatus;
GO CREATE FUNCTION dbo.ProjectEmployeeStatus() RETURNS TABLE AS RETURN (
...
END CATCH
END;
GO DROP PROCEDURE IF EXISTS sp_UpdatePrivateInfo;
GO CREATE PROCEDURE sp_UpdatePrivateInfo @id INT,
    @private_email NVARCHAR(255),
    @private_phone VARCHAR(25) AS BEGIN
SET NOCOUNT ON;
...
1;
END;
GO --CREATE USER testUser WITHOUT LOGIN;
